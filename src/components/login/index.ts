...
export async function makeLogin (page: Page, cpfcnpj : string, password: string) {
  ...
  await page.type('#formBody_formBody_txtUser', cpfcnpj)
  await page.type('#formBody_formBody_txtPassword', password)
  if (!await verifyCaptcha(page)) return makeLogin(page, cpfcnpj, password)
